...
import random, stripe, os, sendgrid, locale
...
def payment_complete():
  ...
  db.session.commit()
  session_cost = locale.format("%d", session.cost, 'en_US')
  print(session_cost)
  try:
    to_email = To(f"kevokagwima@gmail.com, {current_user.email}")
    content = Content("text/plain", f"Congratulations! {current_user.first_name} {current_user.second_name} your medical bill of Ksh {session_cost} has successfully been cleared.\nYour transaction ID is {new_transaction.transaction_id}. Your session and appointment with Dr. {doctor.first_name} {doctor.second_name} has officially been closed.\n\nHead over to your portal to view the session details.")
    mail = Mail(from_email, to_email, subject, content)
    mail_json = mail.get()
    response = sg.client.mail.send.post(request_body=mail_json)
    print(response.headers)
