...
class patient_registration(FlaskForm):
  ...
  def validate_phone_number_validity(self, phone_to_validate):
    phone_number = Patients.query.filter_by(phone=phone_to_validate.data).first()
    if str(phone_number)[0] != 0 and (str(phone_number)[1] != 7 or str(phone_number)[1] != 1):
...
class doctor_registration(FlaskForm):
  first_name = StringField(label="First Name", validators=[DataRequired()])
  second_name = StringField(label="Second Name", validators=[DataRequired()])
  last_name = StringField(label="Last Name", validators=[DataRequired()])
  age = IntegerField(label="Age", validators=[DataRequired()])
  ...
  def validate_phone_number(self, phone_number_to_validate):
    phone_number = Doctors.query.filter_by(phone=phone_number_to_validate.data).first()
    if phone_number:
      raise ValidationError("Phone Number already exists, Please try another one")
  ...
  def validate_email_address(self, email_to_validate):
    email = Doctors.query.filter_by(email=email_to_validate.data).first()
    if email:
      raise ValidationError("Email Address already exists, Please try another one")
